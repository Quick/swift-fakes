...
extension Spy {
    ...
    public func callAsFunction<Value>(_ arguments: Arguments, pendingFallback: Value, pendingDelay: TimeInterval = PendableDefaults.delay) async -> Value where Returning == Pendable<Value> {
    ...
    /// Because of how ``Pendable`` currently works, you must provide a fallback option for when the Pendable is pending.
    /// Alternatively, you can use the throwing version of `callAsFunction`, which will thorw an error instead of returning the fallback.
    public func callAsFunction<Value>(pendingFallback: Value, pendingDelay: TimeInterval = PendableDefaults.delay) async -> Value where Arguments == Void, Returning == Pendable<Value> {
    ...
    public func callAsFunction<Value>(_ arguments: Arguments, pendingDelay: TimeInterval = PendableDefaults.delay) async throws -> Value where Returning == Pendable<Value> {
    ...
    /// - parameter pendingDelay: The amount of seconds to delay if the `Pendable` is .pending before
    /// throwing a `PendableInProgressError`. If the `Pendable` is .finished, then this value is ignored.
    public func callAsFunction<Value>(pendingDelay: TimeInterval = PendableDefaults.delay) async throws -> Value where Arguments == Void, Returning == Pendable<Value> {
        return try await call(()).resolve(delay: pendingDelay)
    }

    // Returning == ThrowingPendable
    /// Records the arguments and handles the result according to ``Pendable.call(delay:)``. This call then throws or returns the success, according to `Result.get`.
    ///
    /// - parameter arguments: The arguments to record.
    /// - parameter pendingDelay: The amount of seconds to delay if the `Pendable` is .pending before
    /// throwing a `PendableInProgressError`. If the `Pendable` is .finished, then this value is ignored.
    public func callAsFunction<Success, Failure: Error>(_ arguments: Arguments, pendingDelay: TimeInterval = PendableDefaults.delay) async throws -> Success where Returning == ThrowingPendable<Success, Failure> {
        return try await call(arguments).resolve(delay: pendingDelay)
    }

    /// Records that a call was made and handles the result according to ``Pendable.call(delay:)``. This call then throws or returns the success, according to `Result.get`.
    ///
    /// - parameter pendingDelay: The amount of seconds to delay if the `Pendable` is .pending before
    /// throwing a `PendableInProgressError`. If the `Pendable` is .finished, then this value is ignored.
    public func callAsFunction<Success, Failure: Error>(pendingDelay: TimeInterval = PendableDefaults.delay) async throws -> Success where Arguments == Void, Returning == ThrowingPendable<Success, Failure> {
        return try await call(()).resolve(delay: pendingDelay)
    }
}
