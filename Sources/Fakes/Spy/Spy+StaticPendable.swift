...
extension Spy {
    ...
    public convenience init<Value>(pendingFallback: Value) where Returning == StaticPendable<Value> {
    ...
    public convenience init() where Returning == StaticPendable<Void> {
        self.init(.pending(fallback: ()))
    }

    /// Create a pendable Spy that is pre-stubbed to return a finished value.
    public convenience init<Value>(finished: Value) where Returning == StaticPendable<Value> {
...
extension Spy {
    ...
    public func stub<Value>(pendingFallback: Value) where Returning == StaticPendable<Value> {
        self.stub(.pending(fallback: pendingFallback))
    }

    /// Update the pendable Spy's stub to be in a pending state.
    public func stubPending() where Returning == StaticPendable<Void> {
    ...
    public func stub<Value>(finished: Value) where Returning == StaticPendable<Value> {
        self.stub(.finished(finished))
    }

    /// Update the pendable Spy's stub to be in a pending state.
    public func stubFinished() where Returning == StaticPendable<Void> {
        self.stub(.finished(()))
    }
}
...
extension Spy {
    ...
    ) async -> Value where Returning == StaticPendable<Value> {
    ...
    public func callAsFunction<Value>(
        pendingDelay: TimeInterval = PendableDefaults.delay
    ) async -> Value where Arguments == Void, Returning == StaticPendable<Value> {
